...
class AIvsHumanObserver {
    ...
    onPlayerMove(data) {
        ...
        // Record move for ML training
        console.log('📹 onPlayerMove called:', data.player, data.move?.row, data.move?.col);
        if (window.gameRecorder) {
            console.log('📹 gameRecorder exists, isRecording:', window.gameRecorder.isRecording);
            window.gameRecorder.recordMove({
                player: data.player,
                move: data.move,
                moveType: data.moveType || data.move.pattern || 'normal',
                reason: data.move.reason || ''
            }, this.gameCore);
        } else {
            console.warn('📹 gameRecorder NOT available!');
        }

        const cellNumber = data.move.row * this.boardSize + data.move.col + 1;
        const playerType = data.isAI ? '🤖 AI' : '👤 Human';
        const playerMoveNum = (this.gameCore.gameHistory || []).filter(m => m.player === data.player).length;
        let message = `${playerType} ${data.player}${playerMoveNum} plays (${data.move.row},${data.move.col}) [cell ${cellNumber}]`;
        message += `\n └─ ${data.move.reason}`;
    ...
    onGameStart(data) {
        ...
        // Notify game recorder
        console.log('📹 onGameStart called, gameRecorder exists:', !!window.gameRecorder);
        if (window.gameRecorder) {
            console.log('📹 Calling startNewGame, isRecording:', window.gameRecorder.isRecording);
            window.gameRecorder.startNewGame();
        } else {
            console.warn('📹 gameRecorder NOT available at game start!');
        }
    ...
    onGameEnd(data) {
        ...
        // Save game recording for ML training
        console.log('📹 onGameEnd called, gameRecorder exists:', !!window.gameRecorder);
        if (window.gameRecorder) {
            console.log('📹 Calling endGame, moves recorded:', window.gameRecorder.currentGameMoves?.length);
            window.gameRecorder.endGame(data.result?.winner, this.gameCore);
        } else {
            console.warn('📹 gameRecorder NOT available at game end!');
        }
    ...
    handleStartGame() {
        ...
        // Notify recorder of new game start (backup in case onGameStart event timing is off)
        if (window.gameRecorder) {
            window.gameRecorder.startNewGame();
        }
