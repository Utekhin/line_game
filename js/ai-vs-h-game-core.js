...
class ConnectionGameCore {
    ...
    makeMove(row, col, player) {
        ...
        // Update diagonal lines SYNCHRONOUSLY before win check - CRITICAL for blocking detection
        if (this.diagonalLinesManager && typeof this.diagonalLinesManager.updateDiagonalLines === 'function') {
            this.diagonalLinesManager.updateDiagonalLines();
        }
    ...
    traceRowByRowPath(player, startingPiece, piecesByPosition) {
        ...
        for (let iteration = 0; iteration < maxIterations; iteration++) {
            ...
            // CRITICAL: Expand within the current row/col by following lateral connections
            // This finds all pieces in the same row/col that connect to our path
            const expandedPieces = this.expandWithinPosition(player, nextRowPieces, piecesByPosition, nextPosition);

            if (expandedPieces.length > nextRowPieces.length) {
                this.log(`🔗 Expanded from ${nextRowPieces.length} to ${expandedPieces.length} pieces via lateral connections in ${player === 'X' ? 'row' : 'col'} ${nextPosition}`);
            }

            // Add new pieces to path and update current position
            fullPath.push(...expandedPieces.filter(p => !fullPath.some(fp => fp.row === p.row && fp.col === p.col)));
            currentRowPieces = expandedPieces;
    ...
    /**
     * Expand within a row/column by following lateral connections
     * This finds ALL pieces in the same row/col that are connected to our path via lateral moves
     */
    expandWithinPosition(player, initialPieces, piecesByPosition, position) {
        if (initialPieces.length === 0) return initialPieces;

        // Get all pieces at this position (row for X, column for O)
        let allPiecesAtPosition;
        if (player === 'X') {
            allPiecesAtPosition = piecesByPosition.byRow[position] || [];
        } else {
            allPiecesAtPosition = piecesByPosition.byCol[position] || [];
        }

        // Start with initial pieces
        const connected = new Set(initialPieces.map(p => `${p.row},${p.col}`));
        const result = [...initialPieces];

        // Keep expanding until no new pieces found
        let changed = true;
        while (changed) {
            changed = false;

            for (const candidate of allPiecesAtPosition) {
                const key = `${candidate.row},${candidate.col}`;
                if (connected.has(key)) continue; // Already in our set

                // Check if candidate is laterally adjacent to any piece in our set
                for (const existing of result) {
                    // Lateral connection: same row OR same col, distance 1 in the other dimension
                    const dr = Math.abs(candidate.row - existing.row);
                    const dc = Math.abs(candidate.col - existing.col);

                    // For X (traversing rows), lateral is same row (dc=1, dr=0)
                    // For O (traversing cols), lateral is same col (dr=1, dc=0)
                    const isLateral = (player === 'X') ?
                        (dr === 0 && dc === 1) :
                        (dc === 0 && dr === 1);
